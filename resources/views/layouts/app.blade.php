...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-aviary-oak text-slate-300 loft-wood overflow-hidden">
        <div class="flex h-screen w-full overflow-hidden" x-data="{ 
            ...
            <div x-show="toast.show" x-cloak
                 ...
                 class="fixed top-20 right-4 z-[100] px-6 py-4 rounded-xl shadow-2xl font-black font-industrial italic border-2 border-aviary-brass/30 bg-aviary-timber text-white"
                 :class="toast.type === 'success' ? 'border-green-500/50' : 'border-red-500/50'">
                <span x-text="toast.message"></span>
            </div>

            <!-- Sidebar -->
            <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-aviary-oak/95 border-r border-aviary-brass/20 transform lg:translate-x-0 transition-transform duration-300 shadow-2xl galvanized-border"
            ...
            <div class="flex-1 lg:ml-64 flex flex-col h-screen overflow-hidden">
                <!-- Top Header Bar -->
                <header class="h-16 flex items-center justify-between px-4 md:px-6 parchment-panel border-b border-aviary-brass/20 z-40">
                    <div class="flex items-center gap-4 lg:hidden">
                        <span class="font-industrial font-black text-aviary-brass tracking-tighter text-sm italic">LOFT MANAGER</span>
                    </div>
                    
                    <div class="hidden lg:block">
                        @if(isset($header))
                            <h2 class="font-industrial font-black text-xl text-white tracking-widest uppercase italic">
                                {{ $header }}
                            </h2>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-3 md:gap-6">
                        ...
                        <div class="flex items-center gap-2 md:gap-3">
                            <span class="hidden sm:block text-[10px] font-black uppercase tracking-tighter text-aviary-feather/60">{{ Auth::user()->name }}</span>
                            <div class="w-8 h-8 rounded-full bg-aviary-brass flex items-center justify-center font-black text-white text-xs shadow-lg shadow-aviary-brass/20">
                ...
                <!-- Scrollable Viewport -->
                <main class="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 pb-24 lg:pb-8 bg-aviary-oak/50">
                    {{ $slot }}
                </main>
            </div>
